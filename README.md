...
Списки задач и покупок с поддержкой нескольких списков, категорий/тегов, быстрых действий и виджетов Android.

[![CI](https://github.com/yuriy-bak/ListYB)](https://github.com/yuriy-bak/ListYB/actions/workflows/flutter_ci.yml)
!License: MIT

## Возможности
- Несколько списков, категории, теги, быстрые теги (позже)
- Фильтры/представления, поиск (позже)
- Редактирование, удаления, отметка выполнения
- Виджет(ы) Android для быстрого доступа
- Экспорт/импорт (JSON), QR (позже)
- Локализация (заготовка), темы (позже)
- Локальное хранение данных (без облака на данный момент)
- Облако для совметной работы и бекапов (позже)
...
## Запуск тестов
Shellflutter test --coverageПоказать больше строк

## Дорожная карта

 Undo/Redo (10 шагов)
 Горячие клавиши (Web/Desktop)
 Голосовой ввод, шаблоны списков, бейджи
 Облачная синхронизация и совместное использование (позже)

Вклад
PR и issues приветствуются. См. CONTRIBUTING.
Лицензия
MIT — см. LICENSE.
